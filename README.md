...
- **0.6.0** ~~Abuse-resistant continuity (IP-bounce penalty). One device key and one browser fingerprint hopping across a pool of IPs at machine speed is the rotating-proxy bot signature — not a person — so that pattern now earns a durable penalty instead of quietly banking trust. Detection is velocity-based and fingerprint-gated: a real person roaming between home, café, and mobile keeps their continuity, and only same-browser IP-hopping at inhuman rates trips it. The penalty can't be washed out by farming cheap clean requests, and it fades over weeks rather than branding a key for good.~~ [Released 2026-05-29]
