...
| Biscuit root signing key (`:biscuit`, 32-byte Ed25519 seed) | **HIGH** (leak ⇒ forge capability tokens at any tier/audience) | **HIGHEST** (the offline-verification trust anchor) | MEDIUM (rotating it invalidates every outstanding token under the old pubkey) |
| Issued capability token (Biscuit, held by the caller) | LOW (public material — asserts only identity/tier/audience, no secret) | **HIGH** (forgery/tampering must be detected — Ed25519 signature chain) | LOW (short TTL is the availability model; expiry is the revocation) |
...
| T21 | Capability-token abuse — theft, replay, over-broad mint, or offline forgery | Authenticity of the offline authorization decision; least-privilege of the granted capability | A `/v1/issue-token` token is a bearer Biscuit a host verifies offline. Defense in depth: (1) **forgery** is blocked by the Ed25519 signature chain rooted at the server's `:biscuit` key — a token under a different key fails `from_b64url`, and the root public key is published read-only at `GET /v1/token-pubkey` for hosts to pin; (2) **over-broad mint** is structurally impossible — the token asserts only the caller's *recomputed* tier (same projection as `/verify`) plus identity/audience/expiry; there is no caller-supplied capability the server signs, and a tier whose cap is 0 (e.g. `:banned`) cannot mint at all; (3) **mint abuse / repurposing** is gated by the device-signed envelope, route-bound to `/v1/issue-token` with `body-sha256 = sha256(issue-token-intent(audience, ttl_ms))`, so a captured envelope cannot mint for a different audience, and the nonce cache makes each request single-use (`E_REPLAY`); (4) **theft / replay of a minted token** is bounded by a short, tier-capped TTL (`check if time($t), $t <= expiry`, default ≤5 min) and the `audience` fact (a token for host A does not satisfy host B's `allow if audience("B")`). The reclaim/verify hot path is untouched — issuance is stateless (no DB write, no schema change). | `src/.../crypto/biscuit_token.clj` (`mint`, fact-injection guard), `src/.../http/handlers/issue_token.clj` (tier recompute, ttl clamp, audience binding), `src/.../http/handlers/token_pubkey.clj`; `test/.../crypto/biscuit_token_test.clj` `expired-token-is-denied-offline`, `wrong-root-pubkey-is-rejected`, `mint-rejects-fact-injection`; `test/.../http/integration_test.clj` `issue-token-returns-offline-verifiable-token`, `issue-token-audience-binding-enforced`, `issue-token-replay-rejected`, `issue-token-clamps-ttl-to-tier-cap`, `verify-response-unchanged-no-token-field`. |
...
- **T21 capability-token revocation (the short-TTL gap)**: an issued Biscuit is verified offline, so there is no way to revoke it before its expiry without a revocation list the host would have to consult — which would reintroduce the per-request network round-trip the token exists to remove. v0.5.0 leans entirely on a short, tier-capped TTL (default ≤5 min) as the availability/revocation model; a stolen token is usable only until it expires, and `audience` binding limits *where*. A revocation-list / token-introspection endpoint (or shorter-lived tokens with a refresh) is deferred and would be added only if a concrete threat (e.g. instant lockout of a compromised caller) demands it. Token linkability via the `identity("…")` fact is no worse than `/verify` (which already returns `identity_ref` to the host); a privacy opt-out is deferred.
