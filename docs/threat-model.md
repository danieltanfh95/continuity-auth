...
| T19 | Filesystem-resident private key (non-browser clients) | Confidentiality of private key | A read of `$CAUTH_HOME/key.pem` exfiltrates the key. The score model treats "same key, many IPs in a short window" as anomalous and demotes via `:fp-mismatch` / `:ip-mismatch` / `:all-mismatch` deltas (see `:scoring` in `resources/config.edn`), so a stolen key cannot indefinitely impersonate the owner — but the detection window is minutes to hours, not instant. Operator guidance: mode 0700 on $CAUTH_HOME, ramdisk for prod, treat as long-lived API credential, rotate on suspected compromise. Hardware-anchored keys (TPM/Secure Enclave/Android Keystore) are out of scope for v0.1. | `docs/non-browser-clients.md` "Threat model — keys on disk"; same score model as T11. |
