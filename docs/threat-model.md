...
- **T14 bootstrap rate limit**: a per-IP flat-quota limiter on `/v1/bootstrap` is implemented (`wrap-bootstrap-rate-limit` in `http/middleware.clj`, default 5 bootstraps per 60s per IP, in-memory atom, single-instance). The flat quota bounds throwaway-identity creation per IP but does not adapt to sustained abuse — an attacker behind one IP can mint 5 identities/minute indefinitely. The v1.1 upgrade is per-IP exponential backoff with a cap (e.g. 1s → 2s → 4s → … → 60s, halving on a quiet interval), so a legitimate user behind shared NAT pays at most one cap-interval before bootstrapping while a parallel-compute attacker on one network sees their per-IP rate collapse asymmetrically. Cross-instance fairness requires backing the limiter with the Datalevin `ratelimit/window.clj` infrastructure rather than the per-instance atom. Per-IP aggregation is acceptable at bootstrap specifically because the false-positive cost is bounded (one delay before bootstrap, no impact on existing identities from that IP); the rest of the protocol keeps IP advisory.
